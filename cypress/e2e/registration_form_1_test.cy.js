...
describe('This is first test suite, Janeli Uuesoo', () => {
    
    it('User can submit data only when valid mandatory values are added', () => {
        //April 24, 2024
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Janeli')
        cy.get('#lastName').type('Uuesoo')
        cy.get('input[name="password"]').type('Janelispasstest3')
        cy.get('[name="confirm"]').type('Janelispasstest3')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        cy.get('#username').type('Cool_Username')
        cy.get('[data-testid="phoneNumberTestId"]').type("10203040")
        cy.get("input[name='password']").type('Password123')
        cy.get('[name="confirm"]').type('Password123')
        cy.get('#firstName').type('Janeli')
        ...
        // Assert that phone nr has tooltip with error message
        cy.get('#phoneNumber').should('have.attr', 'title').should('contain', 'Add phone number')

        //Error messages not included for assignment #3 tests
        cy.get('#input_error_message').should('not.be.visible')
    })
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        // Assert that password/confirmation does not have tooltip
        cy.get("input[name='password']").should('not.have.attr', 'title')
        cy.get("input[name='confirm']").should('not.have.attr', 'title')
    ...
    it('User cannot add letters to phone number', () => {
        ...
        // Assert that submit button is not enabled and that successful message is not visible
        cy.get('#username').type('Cool_Username')
        cy.get('[data-testid="phoneNumberTestId"]').click()
        cy.get("input[name='password']").type('Password123')
        cy.get('[name="confirm"]').type('Password123')
        cy.get('#firstName').type('Janeli')
        cy.get('#lastName').type('Uuesoo')

        // Typing letters to phone nr field
        cy.get('#phoneNumber').scrollIntoView()
        cy.get('#phoneNumber').type('randomtext')
        cy.get('p').contains('NB!').click()
        ...
        cy.get('#success_message').should('not.be.visible')
         ...
         // Error messages not included for assignment #3 tests/fields
         cy.get('#input_error_message').should('not.be.visible')
    })
})